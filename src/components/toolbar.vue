...
<template>
  <div id="toolbar-image">
    <v-toolbar height="40px" elevation="0" class="transparent" dense>
      ...
      <img
        class="tbtitle hidden-xs-only"
        id="stellarium-web-toolbar-logo"
        src="@/assets/images/logo.svg"
        width="33"
        height="33"
        alt="Stellarium Web Logo"
      />
      <span class="tbtitle hidden-xs-only">QUARCS</span>
      <img
        class="tbtitle hidden-xs-only"
        id="stellarium-web-toolbar-logo"
        ...
        alt="Stellarium Web Logo"
      />
      <span class="tbtitle hidden-xs-only">Lithium</span>
      <v-spacer></v-spacer>
      <!---->
      <target-search v-show="false"></target-search>
      <v-spacer></v-spacer>
      ...
      >
        <img
          :src="require(`@/assets/images/svg/ui/${button.icon}.svg`)"
          :alt="button.alt"
          height="25"
        />
      </button>
      ...
      <button class="ScheduleBtn" @click="toggleSchedulePanel">
        ...
        >
          <img
            src="@/assets/images/svg/ui/schedule_table.svg"
            height="25px"
      ...
      <!-- WiFi状态指示 -->
      <div class="wifi-icon">
        <img :src="wifiIcon" alt="WiFi status" height="30" @click="toggleScanner" />
      </div>

      <div>
        <div
          v-if="$store.state.showFPS"
          class="subheader text-subtitle-2 pr-2"
          style="user-select: none"
        >
          FPS {{ $store.state.stel ? $store.state.stel.fps.toFixed(1) : "?" }}
        </div>
        <div class="subheader text-subtitle-2" style="user-select: none">
          FOV {{ fov }}
        </div>
      </div>
      <v-menu
        v-if="$store.state.showTimeButtons"
...
export default {
      ...
      buttons: [
        {
          icon: "sheying",
          alt: "摄影",
          action: this.togglePhotographyPanel,
        },
        {
          icon: "mount",
          alt: "赤道仪",
          action: this.toggleMountPanel,
        },
        {
          icon: "AutoFocus",
          alt: "电调",
          action: this.toggleAutoFocusPanel,
        },
        {
          icon: "Guider",
          alt: "导星",
          action: this.toggleGuiderPanel,
        },
        {
          icon: "Solve",
          alt: "解析",
          action: this.toggleSolvePanel,
        },
        {
          icon: "Home",
          alt: "设置",
          action: this.toggleSettingPanel,
        },
      ],
    };
  },
  ...
  methods: {
    ...
    toggleScanner() {
      this.$store.state.showScanner = !this.$store.state.showScanner;
    },
    ...
    togglePhotographyPanel() {
      if (!this.$store.state.showDeviceSettingsDialog_MainCamera) {
        this.$store.state.showDeviceSettingsDialog_MainCamera = true;
      } else {
        this.$store.state.showDeviceSettingsDialog_MainCamera = false;
      }
    },
    toggleMountPanel() {
      if (!this.$store.state.showDeviceSettingsDialog_Mount) {
        this.$store.state.showDeviceSettingsDialog_Mount = true;
      } else {
        this.$store.state.showDeviceSettingsDialog_Mount = false;
      }
    },
    toggleAutoFocusPanel() {
      if (!this.$store.state.showDeviceSettingsDialog_Focuser) {
        this.$store.state.showDeviceSettingsDialog_Focuser = true;
      } else {
        this.$store.state.showDeviceSettingsDialog_Focuser = false;
      }
    },
    toggleGuiderPanel() {
      if (!this.$store.state.showDeviceSettingsDialog_Guider) {
        this.$store.state.showDeviceSettingsDialog_Guider = true;
      } else {
    ...
    toggleSettingPanel() {
      this.$bus.$emit("toggleSettingPanel");
    },
  },
