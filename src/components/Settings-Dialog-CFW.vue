<template>
  <v-dialog max-width="300" v-model="$store.state.showDeviceSettingsDialog_CFW">
    <!-- showDeviceSettingsDialog_Mount -->
    <v-card
      v-if="$store.state.showDeviceSettingsDialog_CFW"
  ...
  methods: {
    ...
    SetCFWPositionMax(max) {
      ...
      for (let i = 0; i < max; i++) {
        ...
        const cfwIndex = this.configurationItems.findIndex(
