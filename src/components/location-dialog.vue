...
<template>
  <v-dialog max-width="600" v-model="$store.state.showLocationDialog">
    <v-container
      v-if="$store.state.showLocationDialog"
      class="secondary white--text"
    >
      <v-card color="secondary" flat>
        <v-switch
          :label="$t('Use Autolocation')"
      ...
      ></v-text-field>
      <v-btn @click="saveManualCoordinates" color="primary"
        >Save Manual Coordinates</v-btn
      >
    </v-container>
  </v-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    saveManualCoordinates: function () {
      const [lat, lng] = this.manualCoordinates
        .split(",")
