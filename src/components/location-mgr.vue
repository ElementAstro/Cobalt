    ...
    <v-row justify="space-around">
      <v-col cols="4" v-if="doShowMyLocation">
        <v-list two-line subheader>
          ...
          <v-list-item
            href="javascript:;"
            v-for="item in knownLocations"
            v-bind:key="item.id"
            @click.native.stop="selectKnownLocation(item)"
            :style="
              item &&
              knownLocationMode &&
              selectedKnownLocation &&
              item.id === selectedKnownLocation.id
        ...
        <v-card class="blue-grey darken-2 white--text">
                ...
                <v-col>
                  <div>
                    ...
                    >
                      {{
                        locationForDetail
                          ? locationForDetail.short_name +
                            ", " +
                            locationForDetail.country
                          : "-"
                      }}
                    </div>
                    <v-btn
                      ...
                      {{ $t("Use this location") }}</v-btn
                    >
                    <div
                      class="grey--text text-subtitle-2"
                      v-if="locationForDetail.street_address"
                    >
                      {{
                        locationForDetail
                          ? locationForDetail.street_address
                            ? locationForDetail.street_address
                            : $t("Unknown Address")
                          : "-"
                      }}
                    ...
                    <div class="grey--text text-subtitle-2">
                      {{
                        locationForDetail
                          ? locationForDetail.lat.toFixed(5) +
                            " " +
                            locationForDetail.lng.toFixed(5)
                          : "-"
                      }}
                    </div>
                  </div>
          ...
          <div style="height: 375px">
            <v-btn
              ...
              fab
              class="mx-0 pa-0"
              @click.native.stop="centerOnRealPosition()"
              style="
                position: absolute;
                z-index: 10000;
                bottom: 16px;
            ...
            >
              ...
              ></l-circle>
              <l-marker
                v-if="pickLocationMode && pickLocation"
                :lat-lng="[pickLocation.lat, pickLocation.lng]"
                :draggable="true"
                @dragend="dragEnd"
                ><l-tooltip
                  ><div class="black--text">Drag to adjust</div></l-tooltip
                ></l-marker
              >
            </l-map>
          </div>
    ...
    this.$nextTick(() => {
      ...
      var geocoder = new L.Control.Geocoder({
        ...
        const city = res.address.city
          ...
          : res.name;
        loc.short_name =
          pos.accuracy > 500 ? that.$t("Near {0}", [city]) : city;
    ...
    setPickLocation: function (loc) {
      if (loc.accuracy < 100) {
        for (const l of this.knownLocations) {
          const d = swh.getDistanceFromLatLonInM(
            l.lat,
            l.lng,
            loc.lat,
            loc.lng
